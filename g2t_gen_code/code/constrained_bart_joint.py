...
class EncoderStructureLayer(nn.Module):
    ...
    def forward(
        ...
        x_ori = hidden_states  # .transpose(0, 1)  # batch * len * embed_dim already transposed in self attention
...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            embed_input = input_ids
            input_ids = input_ids.view(-1, input_ids.shape[-1])
        elif inputs_embeds is not None:
            embed_input = inputs_embeds[:, :, -1]
        else:
            raise ValueError("You have to specify either input_ids or inputs_embeds")
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale

        embed_pos = self.embed_positions(embed_input)
        ...
        if attention_mask is not None:
            ...
            # in transformer v4, invert in in this
...
class DecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.encoder_attn = SelfAttention(self.embed_dim, config.decoder_attention_heads,
                                          dropout=config.attention_dropout, encoder_decoder_attention=True, )
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, layer_state = self.self_attn(
            hidden_states=hidden_states,
        ...
        if encoder_hidden_states is not None:
            ...
            layer_state = layer_state + cross_attn_present_key_value
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(embed_input) * self.embed_scale
        ...
        # embed positions
        positions = self.embed_positions(embed_input, past_key_values_length)

...
if __name__ == '__main__':
    ...
    # cls model directory
    cls_state_dict = torch.load(
        "electra_small_webnlg20.5pred.25ent.25both.1drop.1reverse.01augment0.9862945289091111.pt")
    ...
    baseline_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/webnlg20t5_jointgt_full/webnlg_baseline_joint_predictions.txt")
    # input2: generation with constraints by lines
    constrained_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/webnlg20t5_jointgt_full/webnlg_5beam.15weight_predictions.txt")
    output_list = []
    # dataset file
    with open("webnlg20/test.json", 'r') as load_f:
        ...
        for each_num in cls_baseline_by_num:
            triple_len_list[each_num] = len(cls_baseline_by_num[each_num])
    ...
    # output file name
    with open('WEBNLG20result.csv', 'w') as csvfile:

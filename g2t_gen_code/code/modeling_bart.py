...
class EncoderStructureLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn = SelfAttention(
            self.embed_dim, config.encoder_attention_heads, dropout=config.attention_dropout, )
        # different: a structure attention
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False,
                ...
                node_attention_mask=None, edge_attention_mask=None, adj_matrix=None):
        residual = x
        if self.normalize_before:
            x = self.self_attn_layer_norm(x)
        x, attn_weights = self.self_attn(
        ...
        adj_matrix_emb = torch.gather(x_edge, 1, adj_matrix_tmp).view(
            -1, node_len, node_len, emb_dim)  # batch * node_len * node_len * emb_dim
        adj_matrix_emb = adj_matrix_emb.contiguous().view(-1, node_len * node_len, emb_dim).transpose(
            0, 1)  # (node_len * node_len) * batch * emb_dim
...
class SelfStructureAttention(nn.Module):
    ...
    def forward(
        ...
        rel_k = rel_k.contiguous().view(bsz * self.num_heads, tgt_len, -1,
...
class BartStructureModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        padding_idx, vocab_size = config.pad_token_id, config.vocab_size
        self.shared = nn.Embedding(vocab_size, config.d_model, padding_idx)
        # use structure encoder and input has edges and nodes,
...
class MyBartPretrain(BartForConditionalGeneration):
    ...
    def forward(self, input_ids, attention_mask=None, encoder_outputs=None, encoder_label=None,
                ...
                input_node_ids=None, input_edge_ids=None, node_length=None, edge_length=None,
                adj_matrix=None, decoder_whole_ids=None, word_length=None,
        ...
        outputs = self.model(
            ...
            decoder_input_ids=_decoder_input_ids, decoder_attention_mask=decoder_attention_mask,
            input_node_ids=input_node_ids, input_edge_ids=input_edge_ids, node_length=node_length,
            edge_length=edge_length, adj_matrix=adj_matrix,
            decoder_cached_states=decoder_cached_states, use_cache=use_cache, )
...
class MyBartForConditionalGeneration(BartForConditionalGeneration):
    ...
    def forward(self, input_ids, attention_mask=None, encoder_outputs=None,
                ...
                decoder_whole_ids=None, decoder_cached_states=None, use_cache=False, is_training=False):
        if is_training:
            _decoder_input_ids = shift_tokens_right(decoder_input_ids, self.config.pad_token_id)
        else:
            _decoder_input_ids = decoder_input_ids
    ...
    def generate(
        ...
        r""" Generates sequences for models with a LM head. The method currently supports greedy decoding, beam-search
        ...
        Parameters:
            ...
            early_stopping: (`optional`) bool
                if set to `True` beam search is stopped when at least `num_beams` sentences finished per batch. Defaults
                to `False` as defined in `configuration_utils.PretrainedConfig`.
            ...
            bad_words_ids: (`optional`) list of lists of int
                `bad_words_ids` contains tokens that are not allowed to be generated. In order to get the
            ...
            decoder_start_token_id=None: (`optional`) int
                Start token id for the decoder. Defaults to ``decoder_start_token_id`` as defined the model's config or
                to the ``bos_token_id``
        ...
        Examples::
            ...
            # generate 3 independent sequences using beam search decoding (5 beams) with sampling from initial
            # context 'The dog'

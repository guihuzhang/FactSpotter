...
class Beam(object):
    ...
    def get_hypothesis(self, k):
        ...
        for j in range(len(self.prev_ks) - 1, -1, -1):
            #  convert K index to long to solve bug
...
class GraphGatedTransformerDecoder(nn.Module):
    ...
    def encode(self, input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask,
               gather_level_mask):
        # -- Encode source
    ...
    def forward(self, input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask,
                gather_level_mask, tgt_seq, positionwise_copy_prob=False):
        enc_output = self.encode(input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask,
                                 gather_level_mask)
        # -- Prepare masks
        non_pad_mask = get_non_pad_mask(tgt_seq, self.pad_idx)
    ...
    def greedy_decode(self, input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask,
                      gather_level_mask, max_token_seq_len=30):
        enc_output = self.encode(input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask,
                                 gather_level_mask)
...
def load_my_state_dict(self, state_dict):
    #  copy my old loading code but it doesn't work very well
    # print(len({k: v for k, v in state_dict.items()}))
    # pretrained_state_dict = {k: v for k, v in state_dict.items() if k in self.state_dict() and
    #                          self.state_dict()[k].size() == v.size()}
    # print(len(pretrained_state_dict))
    ...
    for name, param in state_dict.items():
        if 'post_word_emb' in name:
            ...
            continue
        elif 'entity_embeddings' in name or 'triple_embeddings' in name:

...
if __name__ == '__main__':
    ...
    # cls model directory
    cls_state_dict = torch.load("electra_dart.5pred.25ent.25both.1drop0.9921646746347942.pt")
    ...
    prevent_predicates = ["class name", "entity name", "domains", "literal name", "sentence category",
                          "Sentence Data Source", "Title", "[title]"]
    ...
    baseline_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/dart_t5small_new/dart_new_baseline_t5s_predictions.txt")
    # input2: generation with constraints by lines
    constrained_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/dart_t5small_new/dart_new_t5s.15predictions.txt")

    not_total_positive_list = []
    output_list = []
    # dataset file
    with open("dart_kun/test_new.json", 'r') as load_f:
        ...
        print("Baseline CLS", np.mean(cls_baseline), "Baseline CLS Binary", np.mean(cls_baseline_binary),
              "BERT", np.mean(bert_r_baseline), "BART", np.mean(bart_baseline), "BLEURT", np.mean(bleurt_baseline),
              coco_eval_base.eval)
    ...
    # output file name
    with open('DartRedoT5B.15.csv', 'w') as csvfile:

...
if __name__ == '__main__':
    ...
    # cls model directory
    cls_state_dict = torch.load("electra_grail_fix.5pred.25ent.25both.1drop0.9670569557451876.pt")
    cls_model.load_state_dict(cls_state_dict)
    cls_model = cls_model.to(torch.device("cuda"))
    cls_model.eval()

    prevent_predicates = ["class name", "entity name", "domains", "literal name"]
    ...
    baseline_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/grail_new_t5jointgt/grail_iid_baseline_t5jointgt_predictions.txt")
    # input2: generation with constraints by lines
    constrained_res = read_gen_result(
        "/mnt/disk2/zhangkun/codes/emnlp23zk_test_files/grail_new_t5jointgt/grail_iid_t5joint_beam5weight.15predictions.txt")
    output_list = []
    # dataset file
    with open("grail_new/iid.json", 'r') as load_f:
        ...
        for idx in range(0, len(gt_list)):
            sentence_id = gt_list[idx]["id"]
            triple_list = []
            entity_dict = {}
            all_constraint_prompt = set()
            for each_triple in gt_list[idx]['kbs']:
                tmp_subject = gt_list[idx]["kbs"][each_triple][0]
                for each_po in gt_list[idx]['kbs'][each_triple][2]:
                    ...
                    if each_po[0] in property_predicates:
            ...
            for each_prompt in all_constraint_prompt:
                gt_cls_input.append(each_prompt + gt.lower().strip(",? "))
                tmp_base = baseline_res[idx]
                # tmp_base = baseline_res[sentence_id]
                baseline_cls_input.append(each_prompt + tmp_base.strip(",? "))
                tmp_con = constrained_res[idx]
                # tmp_con = constrained_res[sentence_id]
        ...
        for each_id in cls_constraint_dict:
            ...
            if tmp_len not in len_dataset_dict:
                len_dataset_dict[tmp_len] = 1
            else:
                len_dataset_dict[tmp_len] += 1
        ...
        print("Baseline CLS", np.mean(cls_baseline), "Baseline CLS Binary", np.mean(cls_baseline_binary),
        ...
        print("Constraint CLS", np.mean(cls_constraint), "Constraint CLS Binary", np.mean(cls_constraint_binary),
    ...
    # output file name
    with open('GrailZero.csv', 'w') as csvfile:

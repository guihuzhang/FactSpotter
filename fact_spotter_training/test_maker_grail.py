...
def get_param_and_run(drop_percent, neg_pred_percent, output_dir):
    ...
    # train_list = [["augment_dataset/GraphQ_no_zero_predicate.json", "grail"],
    ...
    test_dataset = FixedDatasetMaker([["../grail_new/test.json", "grail"]],
    ...
    # test_loader = PDataLoader(test_dataset, b_size, 1)
    # model = AutoModelForSequenceClassification.from_pretrained(model_name)
    # model_state_dict = torch.load(test_model)
    # model.load_state_dict(model_state_dict)

    # loss_func = torch.nn.CrossEntropyLoss(weight=torch.Tensor([1, 1])).to(torch.device("cuda"))
    # if torch.cuda.is_available():
    #     model.to(torch.device("cuda"))
    # evaluation(model, test_loader, tokenizer, logger, 0)
    # optimizer = AdamW(model.parameters(), lr=learning_rate, amsgrad=True)
    # scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=1, T_mult=2)
    # train_func(logger, model, train_loader, test_loader, optimizer,
